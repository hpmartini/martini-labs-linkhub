...
export type SchwalmMode = "background" | "badge" | "profile" | "off";

interface ThemeContextType {
  theme: Theme;
  schwalmMode: SchwalmMode;
  setSchwalmMode: (mode: SchwalmMode) => void;
}
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  ...
  const [schwalmMode, setSchwalmMode] =
    React.useState<SchwalmMode>("background");
  ...
  useEffect(() => {
    ...
    root.style.setProperty(
      "--canvas-grid-color-rgb",
      theme.canvas.gridColorRgb
    );
  ...
  // Persist mode choice across reloads/theme switches if desired (optional)
  // For now simple state is enough for testing

  return (
    <ThemeContext.Provider value={{ theme, schwalmMode, setSchwalmMode }}>
      {children}
...
export const useSchwalmControl = (): {
  schwalmMode: SchwalmMode;
  setSchwalmMode: (mode: SchwalmMode) => void;
} => {
  const context = useContext(ThemeContext);
  if (!context) {
    throw new Error("useSchwalmControl must be used within a ThemeProvider");
  }
  return {
    schwalmMode: context.schwalmMode,
    setSchwalmMode: context.setSchwalmMode,
  };
};
